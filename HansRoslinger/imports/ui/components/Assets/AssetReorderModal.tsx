...
export default function AssetReorderModal({ isOpen, onClose, assetId, assetName }: Props) {
  ...
  return (
    <Box sx={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3, borderRadius: 2, boxShadow: 24, zIndex: 1300, minWidth: 420, maxHeight: '70vh', overflowY: 'auto' }}>
      <Typography variant="h6" gutterBottom>Reorder images for {assetName}</Typography>
      <List>
        {localOrder.map((id, idx) => {
          ...
          return (
            <ListItem
              ...
              secondaryAction={
              <Box>
                <IconButton aria-label="up" onClick={() => idx > 0 && move(idx, idx - 1)} sx={{ color: 'text.secondary', '&:hover': { color: 'text.primary' } }}>
                  <ArrowUpwardIcon fontSize="small" />
                </IconButton>
                <IconButton aria-label="down" onClick={() => idx < localOrder.length - 1 && move(idx, idx + 1)} sx={{ color: 'text.secondary', '&:hover': { color: 'text.primary' } }}>
