...
const GestureDetector = () => {
  ...
  useEffect(() => {
    ...
    const setup = async (retryCount = 0) => {
      try {
        ...
        if (retryCount < MAX_RETRIES) {
          setTimeout(() => setup(retryCount + 1), RETRY_DELAY_MS);
        } else {
          console.error("Failed to initialize GestureRecognizer after maximum retries.");
        }
      }
    };
    ...
    return () => {
      // Cleanup video stream and interval
    ...
    const detectGesture = async () => {
      ...
      const processFrame = async () => {
        ...
        if (video.readyState === 4) {
          ...
          console.log("1 ", res);
          setGestureResult(res);
          console.log("2 ",gestureResult);
  ...
  useEffect(() => {
    // Code to be called when new gestures are detected gies here
    console.log("HERE", gestureResult);
  }, [gestureResult]);
